...
- ModsExec: выполнить хук из разных мест базового движка. Список хуков находится в интерфейсе `GameMod`. Хук выполняется для всех модов в порядке активации. Вызов без параметров.
...
Модам доступны глобальные переменные игры. Например, мод может поменять характериситики юнитов. Список глобальных переменных:
- GlobalUser
- GlobalUni
- StartPage
- buildmap
- resmap
- fleetmap
- fleetmap_nosat: будет выпилено
- fleetmap_revnosat: будет выпилено
- defmap
- defmap_norak: будет выпилено
- initial
- UnitParam
- RapidFire
