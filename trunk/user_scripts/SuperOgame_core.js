...
ststbl.innerHTML = '<tr><td class="c" colspan="2" style="font-weight:bold">SuperOgame '+xpath('//tr/td/div/font/a[contains(@href,"page=options")]').innerHTML+'</td></tr><tr><td colspan="2" class="c" style="text-align:center">'+i18n('General',54)+'</td></tr>'
+'<tr><th style="text-align:left" width=70%>'+i18n('Global time format',100)+'</th><th><input style="text-align:center" value="'+sofGet('soGen_TimeF')+'" size="19" maxlength="20" onchange="sofSet(\'soGen_TimeF\',this.value)" /></th></tr>'
+addOption('txts','soGen_TimeDiff',i18n('Time Zone',101))
+addOption('color','soGen_notEnC',i18n('Not enough resources color',102))
+addOption('cb2','soGen_Clock',i18n('Display server time clock above left menu',103),'soGen_ClockC','soGen_ClockF')
+addOption('cb','soGen_ClockI',i18n('Switch local/server time for clocks',104))
+addOption('cb2','soGen_Moons',i18n('Highlight moons in planets list',105),'soGen_MoonsC')
+addOption('cb','soGen_ChgButts',i18n('Add planet change buttons',106))
+addOption('cb','soGen_JumpGate',i18n('Display Jump Gate shortcut in left menu',107))
+addOption('cb','soGen_MLst',i18n('Display Member List shortcut in left menu',108))
+'<tr><th style="text-align:left">'+i18n('Member list link sort order',109)+'</th><th><select style="text-align:center" onchange="sofSet(\'soGen_MLstSort1\',this.options[this.selectedIndex].value)"><option '+(sofGet('soGen_MLstSort1') == 0 ? 'selected="selected" style="color:orange"' : '')+' value="0">Coords</option><option '+(sofGet('soGen_MLstSort1') == 1 ? 'selected="selected" style="color:orange"' : '')+' value="1">Name</option><option '+(sofGet('soGen_MLstSort1') == 2 ? 'selected="selected" style="color:orange"' : '')+' value="2">Rank</option><option '+(sofGet('soGen_MLstSort1') == 3 ? 'selected="selected" style="color:orange"' : '')+' value="3">Points</option><option '+(sofGet('soGen_MLstSort1') == 4 ? 'selected="selected" style="color:orange"' : '')+' value="4">Date</option><option '+(sofGet('soGen_MLstSort1') == 5 ? 'selected="selected" style="color:orange"' : '')+' value="5">Status</option></th></tr>'
+'<tr><th style="text-align:left">'+i18n('Member list link sort (ascending/descending)',112)+'</th><th><input type="radio" id="opt0" name="soGen_MLstSort2" value="0" '+(sofGet('soGen_MLstSort2') == '0' ? 'checked="true"' : '')+' onchange="sofSet(\'soGen_MLstSort2\',this.value)" /><label for="opt0" style="cursor:pointer"> ⇧</label> <input type="radio" id="opt1" name="soGen_MLstSort2" value="1" '+(sofGet('soGen_MLstSort2') == '1' ? 'checked="true"' : '')+' onchange="sofSet(\'soGen_MLstSort2\',this.value)" /><label for="opt1" style="cursor:pointer"> ⇩</label></th></tr>'
+addOption('cb','soGen_Banned',i18n('Display Banned shortcut in left menu',110))
+addOption('cb','soGen_CircMes',i18n('Display circular message shortcut in left menu',111))
