...
function EditNote ($note_id)
{
    ...
    echo "<tr><th>Тема</th><th><input type=text name=betreff size=30 maxlength=30 value='".$note['subj']."'></th></tr>\n";
    echo "<tr><th>Заметка (<span id=\"cntChars\">".$note['textsize']."</span> / 5000 символов)</th><th><textarea name=text cols=60 rows=10 onkeyup=\"javascript:cntchar(5000)\">".$note['text']."</textarea></th></tr>\n";
...
{
    if ( $_POST['s'] == 1 ) AddNote ( $GlobalUser['player_id'], $_POST['betreff'], $_POST['text'], $_POST['u'] );
    else if ( $_POST['s'] == 2 ) UpdateNote ( $GlobalUser['player_id'], $_POST['n'], $_POST['betreff'], $_POST['text'], $_POST['u'] );
}
if ( key_exists ('delmes', $_POST) )    // Удалить
{
    foreach ($_POST['delmes'] as $i => $entry) DelNote ( $GlobalUser['player_id'], $i );
}

// Проверить неверные параметры.
if ( key_exists ('a', $_GET) )
{
    if ( $_GET['a'] < 1 || $_GET['a'] > 2 ) die ();
    if ( $_GET['a'] == 2 && !key_exists ('n', $_GET)) die ();
    if ( $_GET['a'] == 2 && LoadNote ( $GlobalUser['player_id'], $_GET['n'] ) === FALSE ) die();
}
...
if ( key_exists ('a', $_GET) )
{
    if ( $_GET['a'] == 1 ) CreateNewNote ();
    else if ( $_GET['a'] == 2 ) EditNote ( $_GET['n'] );
}
else
{
    echo "<form action=\"?page=notizen&session=".$_GET['session']."\" method=post>\n";
    ...
    if ($num)
    {
        while ($num--)
        {
            ...
            echo "  <th><a href=?page=notizen&session=".$_GET['session']."&a=2&n=".$note['note_id']."><font color=$col>".$note['subj']."</font></a></th>\n";
