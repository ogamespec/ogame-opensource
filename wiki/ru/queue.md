...
Главный модуль для управления временным потоком игры. Программный модуль находится в файле `game/queue.php`
...
Главные типы событий аккаунта:
 - Счётчики времени на аккаунте игрока (удаление аккаунта итп.)
...
Объекты дальнего космоса (Farspace) исчезают в понедельник в 1:10 по серверу, если на них или от них не летит ни одного флота (как фантомные ПО). Счётчик посещений объекта дальнего космоса (хранится как его металл) остывает на 3 каждый час (#174), поэтому позицию можно посещать 3 раза в час без повышения её истощения.
...
- FarspaceCooldown: остывание счётчика экспедиций на объектах дальнего космоса (каждый час)
- CleanFarspace: чистка заброшенных объектов дальнего космоса (еженедельно, если на них или от них не летит флот)
...
- Shipyard: задание для верфи (sub_id - номер планеты, obj_id - тип постройки)  :warning: Это особенное событие, требующее специальной обработки (#148)
