...
Боевой движок реализован в виде программы, написанной на Си (бекенд). Со стороны PHP к нему прикручен скрипт battle.php (фронтенд), который подготавливает данные для боя, передает их боевому движку, а также обрабатывает результаты боя и генерирует боевые доклады.
...
- Затем через командную строку запускается программа боевого движка с параметром: battle\_id. (второй параметр командной строки - seed для ДСЧ)
...
- Скрипт battle.php (фронтенд) используя данные расчётов из файла в папке battleresult генерирует боевые доклады и вносит изменения в аккаунты игроков.
