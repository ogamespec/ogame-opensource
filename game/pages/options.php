...
class Options extends Page {
        ...
        // Regular menu
            ...
            if ( method () === "POST" && !key_exists ( 'urlaub_aus', $_POST) ) {
                ...
                if ( $prem['commander'] ) {
                    ...
                    if ($settings_bud) $flags |= USER_FLAG_SHOW_BUDDY_BUTTON; else $flags &= ~USER_FLAG_SHOW_BUDDY_BUTTON;
                    if ($settings_mis) $flags |= USER_FLAG_SHOW_ROCKET_ATTACK_BUTTON; else $flags &= ~USER_FLAG_SHOW_ROCKET_ATTACK_BUTTON;
                    if ($settings_rep) $flags |= USER_FLAG_SHOW_VIEW_REPORT_BUTTON; else $flags &= ~USER_FLAG_SHOW_VIEW_REPORT_BUTTON;
                    if ($settings_folders) $flags |= USER_FLAG_DONT_USE_FOLDERS; else $flags &= ~USER_FLAG_DONT_USE_FOLDERS;
    ...
    public function view () : void {
        ...
        if ( $GlobalUser['validated'] == 0 ) {
            ...
            ?>
             <form action="index.php?page=options&session=<?php echo $session;?>&mode=change" method="POST" >
              <tr> <td class="c" colspan="2">Vacation Mode</td>  </tr>
              <tr>   </tr>
