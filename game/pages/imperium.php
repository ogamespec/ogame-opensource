    ...
    public function controller () : bool {
        ...
        // planettype is always 1 or 3 here (normalized above), so this block always runs.
        {
            ...
            while ($rows--)
            {
                ...
                if ($planet['type'] == PTYP_MOON ) $this->moons++;
                if ( $this->planettype == 1 && $planet['type'] == PTYP_MOON ) continue;
                if ( $this->planettype == 3 && $planet['type'] != PTYP_MOON ) continue;
                $this->plist[$this->num] = GetUpdatePlanet ($planet['planet_id'], $now);
                $this->num ++;
            }
        }
...
<!-- ## -->
            ...
            {
                $sum = 0;
                foreach ( $plist as $j=>$planet ) $sum += $planet[$gid];
                if ( $sum > 0)
                {
                    echo "        <tr height=\"20\">\n";
                    echo "            <th width=\"75\">\n";
