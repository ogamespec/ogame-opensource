    ...
    public function view () : void {
        ...
        foreach ($techtree as $i => $req )
        {
            ...
            foreach ($req['techs'] as $tech) {
                ...
                else $details = "<a href=\"index.php?page=techtreedetails&session=$session&tid=$tech\">[i]</a>";

                echo "<tr> \n";
                echo "<td class=l> \n";
                echo "<table width=\"100%\" border=0 cellspacing=0 cellpadding=0><tr><td align=left><a href=\"index.php?page=infos&session=$session&gid=$tech\">".loca("NAME_$tech")."</a> \n";
                echo "</td><td align=right>$details</td></tr></table></td> \n";

                echo "<td class=l> \n";
                foreach ($requirements[$tech] as $obj => $lvl ) {
                    $ok = $this->MeetRequirement ( $GlobalUser, $aktplanet, $obj, $lvl );
                    if ($ok) $color = '#00ff00';
                    else $color = '#ff0000';
