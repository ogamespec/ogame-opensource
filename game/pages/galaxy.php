...
class Galaxy extends Page {
    ...
    public function controller () : bool {
        ...
        if ( key_exists ('session', $_POST)) $this->coord_g = intval($_POST['galaxy']);
        else if ( key_exists ('galaxy', $_GET)) $this->coord_g = intval($_GET['galaxy']);
        else if ( key_exists ('p1', $_GET)) $this->coord_g = intval($_GET['p1']);
        else $this->coord_g = $aktplanet['g'];
        if ( key_exists ('session', $_POST)) $this->coord_s = intval($_POST['system']);
        else if ( key_exists ('system', $_GET)) $this->coord_s = intval($_GET['system']);
        else if ( key_exists ('p2', $_GET)) $this->coord_s = intval($_GET['p2']);
        else $this->coord_s = $aktplanet['s'];
        if ( key_exists ('session', $_POST)) $this->coord_p = 0;
        else if ( key_exists ('position', $_GET)) $this->coord_p = intval($_GET['position']);
        else if ( key_exists ('p3', $_GET)) $this->coord_p = intval($_GET['p3']);
        else $this->coord_p = $aktplanet['p'];

        if ($this->coord_g < 1 ) $this->coord_g = 1;
        if ($this->coord_g > $GlobalUni['galaxies'] ) $this->coord_g = $GlobalUni['galaxies'];

        if ($this->coord_s < 1 ) $this->coord_s = 1;
        if ($this->coord_s > $GlobalUni['systems'] ) $this->coord_s = $GlobalUni['systems'];
        ...
        $this->not_enough_deut = ( $aktplanet['g'] != $this->coord_g || $aktplanet['s'] != $this->coord_s) && $aktplanet[GID_RC_DEUTERIUM] < GALAXY_DEUTERIUM_CONS;

        // Charge GALAXY_DEUTERIUM_CONS deuterium for viewing a non-home system (regular users only)
        if ( !$this->not_enough_deut && $GlobalUser['admin'] == USER_TYPE_PLAYER )
        {
            if ( $aktplanet['g'] != $this->coord_g || $aktplanet['s'] != $this->coord_s )
            {
    ...
    public function view () : void {
        ...
        $coord_p = $this->coord_p;
        $not_enough_deut = $this->not_enough_deut;
        $nowfleet = $this->nowfleet;
        $maxfleet = $this->maxfleet;
    ...
    <?php
        ...
        <?php
            ...
            if ($num_custom > 0) {
                $this->ShowCustomObjects ($p, $custom_planets);
            }
        ...
        for ($p; $p<=15; $p++) $this->empty_row ($p, $custom_planets);
    ...
    // Display custom Galaxy objects added by mods.
    private function ShowCustomObjects (int $p, array $custom_planets) : void {
    ...
    private function empty_row (int $p, array $custom_planets) : void {
    ...
    private bool $not_enough_deut = false;
