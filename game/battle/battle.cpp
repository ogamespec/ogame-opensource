...
int DoBattle (Slot *a, int anum, Slot *d, int dnum, unsigned long battle_seed, int max_round)
{
    ...
    char * ptr = ResultBuffer, *round_patch;
...
int ParseSlot(Slot* slot, char* lp)
{
    ...
    slot->unit = (UnitCount*)malloc(sizeof(UnitCount) * slot->unit_count);
    if (!slot->unit) {
        free_explode_result(argv);
        return BATTLE_ERROR_INSUFFICIENT_RESOURCES;
    }
...
int SetRapidfire(int enable, char* rftab) {
    ...
    if (Rapidfire) {
        ...
        while (args_left) {
            ...
            if (num_targets != 0) {
                RF[ord].to = (UnitCount*)malloc(sizeof(UnitCount) * num_targets);
                if (!RF[ord].to) {
                    free_explode_result(argv);
                    return BATTLE_ERROR_INSUFFICIENT_RESOURCES;
                }
...
int main(int argc, char **argv)
{
    ...
    // Load the source file and select the source data
    {
        ...
        battle_data = (char *)FileLoad ( filename, NULL, "rt" );
        if (!battle_data) {
