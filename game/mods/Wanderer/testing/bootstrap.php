...
// Bootstrap for the Wanderer modification's own PHPUnit test suite.
//
// This loads the game core with the in-memory SQLite backend
// (DB_CONNECTION=sqlite, DB_DATABASE=:memory:) so the mod's DB-backed hooks
// can be tested without a MySQL server, then loads the Wanderer modification
// itself (the Wanderer class and its constants).
//
// The mod tests live here, inside the modification, because they are tied to
// a specific modification and should not pollute the repository's main test
// suite.
...
// Load the Wanderer modification (Wanderer class + constants).
require_once __DIR__ . '/../../../../game/mods/Wanderer/main.php';
